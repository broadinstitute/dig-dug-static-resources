...
(function() {
  ...
  baget.manhattan = function() {
    ...
    var width = 1,
      ...
      xAxisAccessor = function(x) {
        return x;
      },
      yAxisAccessor = function(y) {
        return y;
      },
      chromosomeAccessor = function(c) {
        return c;
      },
      nameAccessor = function(c) {
        return c;
      },
      ...
      chromosomes = (function(color) {
        ...
        var chromosomeInfo = [
          ...
          activeGenomeLength = function() {
            var skipThis = includeXChromosome
              ? chromosomes.lengthOfAChromosome("X")
              : 0;
            skipThis += includeYChromosome
              ? chromosomes.lengthOfAChromosome("Y")
              : 0;
          ...
          convertALocation = function(chromosomeName, position) {
            ...
            if (typeof chromosomeIndex !== "undefined") {
              ...
              returnValue =
                (startingPosition * activeGenomeLength()) / 100.0 + position; // start of chromosome
            }
     ...
     */
    var determineDataExtents = function(
      allData,
      crossChromosomePlot, // Boolean-- X extent equals entire human chromosome
      ...
      if (crossChromosomePlot) {
        // plot the whole genome, so we know the extents
        ...
        (function() {
          ...
          for (var i = 0; i < numberOfExtents; i++) {
            rememberExtents.push(
              chromosomes.convertALocation(
                "" + chromosomeAccessor(allData[i]),
                xAxisAccessor(allData[i])
              )
            );
          }
      ...
      return {
        minimumXExtent: minimumExtent,
        maximumXExtent: maximumExtent,
        minimumYExtent:
          typeof minimumYValueOverride !== "undefined"
            ? minimumYValueOverride
            : d3.min(allData, function(d) {
                return yAxisAccessor(d);
              }),
        maximumYExtent:
          typeof maximumYValueOverride !== "undefined"
            ? maximumYValueOverride
    ...
    var zoomed = function() {
      ...
      selection
        .selectAll(".significanceIndicator")
        .attr("x1", function(d) {
          return x(0);
        })
        .attr("y1", function(d) {
    ...
    var createAxes = function(
      ...
      for (var i = 1; i < chromosomes.numberOfAutosomes + 1; i++) {
        v.push(
          ((chromosomes.chromosomeInfo[i - 1].p +
            chromosomes.chromosomeInfo[i].p) *
            chromosomes.activeGenomeLength()) /
            200
        );
      }
      if (includeXChromosome) {
        v.push(
          ((chromosomes.chromosomeInfo[21].p +
            chromosomes.chromosomeByName("X").p) *
            chromosomes.activeGenomeLength()) /
            200
        );
      }
      if (includeYChromosome) {
        v.push(
          ((chromosomes.chromosomeInfo[22].p +
            chromosomes.chromosomeByName("Y").p) *
            chromosomes.activeGenomeLength()) /
            200
        );
      }
    ...
    var createDots = function(
      ...
      var anchors = dotHolder.selectAll("a.dot").data(data, function(d) {
        // merge data sets so that we hold only unique points
        return (
          "" +
          chromosomeAccessor(d) +
          "_" +
      ...
      var dots = anchors
        ...
        .style("fill", function(d, i) {
          if (yAxisAccessor(d) > significanceThreshold) {
            return chromosomes.colorSignificanceByChromosomeNumber(
              chromosomeAccessor(d)
            );
          } else {
      ...
      if (typeof yValueThreshold !== "undefined") {
        for (var i = 1; i < chromosomes.chromosomeInfo.length; i++) {
          blockGroup
            ...
            .attr(
              "height",
              yScale(dataExtent.minimumYExtent) - yScale(yValueThreshold)
            )
            .style("fill", function(d) {
              return chromosomes.colorByChromosomeNumber(
                chromosomes.chromosomeInfo[i].c
              );
            });
        }
      }
    ...
    var addSignificanceIndicator = function(
      significanceHolder,
      ...
      var significanceIndicator = significanceHolder
        .selectAll("line.significanceIndicator")
        .data([significanceThreshold], function(d) {
          // merge data sets so that we hold only unique points
          return "" + d;
        });
    ...
    var tip = d3
      ...
      .html(function(d) {
        ...
        if (d) {
          if (
            typeof tooltipAccessor !== "undefined" &&
            ...
            textToPresent =
              nameAccessor(d) +
              "<br/>pValue=" +
              Math.pow(10, 0 - pValue).toPrecision(3) +
              "<br/>Chr=" +
              chromosomeAccessor(d) +
              ", pos=" +
              xAxisAccessor(d);
    ...
    instance.initialize = function() {
      ...
      dataExtent = determineDataExtents(
        selection.data()[0],
        crossChromosomePlot,
        overrideXMinimum,
        overrideXMaximum,
    ...
    instance.render = function(currentSelection) {
      ...
      x = d3.scale
        .linear()
        .domain([
          dataExtent.minimumXExtent -
            (dataExtent.maximumXExtent - dataExtent.minimumXExtent) / 50,
          chromosomes.activeGenomeLength()
        ])
